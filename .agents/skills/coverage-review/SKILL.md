...
Use this after `compile`, after major wiki edits, or whenever the user asks whether a topic is "covered enough."

## Input

Use the user's request text to determine scope and mode.

- If a `raw/...` file is specified, evaluate that source
- If a `wiki/sources/...` page is specified, evaluate that source page
- If a post-compile handoff payload is provided, use it as the routing metadata for the review
...
When this skill is invoked from `compile` using a post-compile handoff payload:

- Prefer running through the project-scoped `coverage_reviewer` sub-agent when `.codex/agents/coverage-reviewer.toml` exists
- Run in a fresh agent/session with no shared ingest context
...
- Rebuild all working context by reading the repository again; do not rely on unstated ingest memory
...
### Step 2: Generate primary source-grounded questions

First choose the primary question count `N` based on source complexity:

- Use `5` for short essays, short source notes, or clearly single-claim sources
- Use `8` for standard papers or standard article-length sources
- Use `10-12` for long papers, books, technical reports, or sources that naturally require navigation across many related wiki pages

Then generate exactly `N` questions that satisfy all of the following:
...
The set must include at least these three core facets whenever the source supports them:

- Core claim / thesis
- Main mechanism / reasoning / process
- Key evidence / example / result
...
Adapt the question mix to the source type:

- Research papers: prioritize claim, mechanism, evidence, limitation, comparison, and application
- Argument / essay sources: prioritize thesis, reasoning chain, example, tension or counterpoint, and implication
- Guides / books / design docs: prioritize core thesis, structure, mechanism, workflow or checklist, and tradeoff or boundary

Question style rules:

- Prefer open-ended questions over yes/no checks
- Avoid trivia, citation fishing, and wording that only tests page-memory
- Prefer questions that expose retrieval quality, navigation quality, or synthesis quality
- When a good wiki answer should naturally require moving from the source page to a related concept/entity/comparison page, include at least one such integration question
...
- Answer each of the `N` primary questions as if you were a user relying on the wiki
...
- At least `80%` of primary questions must pass
- Average `page_count` should be `<= 3`
- No failed question should be central to the source's main contribution

Treat the following as central by default:

- The core claim / thesis question
- The main mechanism / reasoning question
- The key evidence / example question, when the source clearly provides one
...
After patching, re-run the same `N` primary questions with the same wiki-only restriction.
...
After a self-healed source passes same-question regression, generate a new holdout set sized to the original review:

- `3` holdout questions for `N = 5`
- `4` holdout questions for `N = 8`
- `5` holdout questions for `N = 10-12`

Holdout questions must satisfy all of the following:

- They are explicitly supported by the same raw source
- They are materially different from the primary questions
...
- Holdout score is at least `75% pass`
...
### Score
- Initial: 6/8 pass, 1 partial, 1 fail
- Regression: 8/8 pass
- Holdout: 3/4 pass
- Avg pages used: 2.4
